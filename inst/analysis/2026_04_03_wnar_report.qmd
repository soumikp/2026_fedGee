---
title: "Real-World Application: Predicting MAUD Initiation"
format:
  html:
    theme: cosmo
    toc: true
...
## Real-World Application: Predicting MAUD Initiation in the VHA

To demonstrate the validity of the Federated Generalized Estimating Equations (FedGEE) algorithm, we applied the method to a national cohort of 29,041 Veterans diagnosed with Alcohol Use Disorder (AUD) across the Veterans Health Administration (VHA). The clinical goal was to identify predictors of initiating Medications for Alcohol Use Disorder (MAUD) following inpatient discharge.

### Methodology and Privacy Preservation
Rather than pooling sensitive Electronic Health Record (EHR) data into a central repository, we emulated a federated network by partitioning data by hospital facility. FedGEE iteratively communicated only aggregated mathematical components (Score and Bread matrices) to a central server, ensuring that no individual-level Veteran data ever left the local site.

### Comparison of Results
As shown in @fig-forest-maud, FedGEE achieved exact point-estimate parity with a theoretically centralized Pooled GEE. However, the confidence intervals differ significantly. 

Standard GEE software often produces anti-conservative (too narrow) confidence intervals in large datasets because it conflates a high patient count with high statistical certainty. In reality, the "true" sample size in hierarchical data is driven by the number of clusters (hospitals). By applying the **Mancl-DeRouen (MD) small-sample correction** and using **t-distribution inference**, FedGEE provides honest, wider confidence bands that appropriately reflect the uncertainty inherent in a finite number of hospital sites.

### Visualization of Predictors

```{r}
#| label: fig-forest-maud
#| fig-cap: "Adjusted Odds Ratios for MAUD Initiation. This plot compares centralized Pooled GEE (blue) with the privacy-preserving FedGEE (orange). Categories and variable ordering strictly follow the study's Table 1 baseline characteristics."
#| fig-width: 10
#| fig-height: 16
#| echo: false
#| warning: false
...
clean_data <- plot_data %>%
  ...
  mutate(
    ...
    Clean_Term = case_when(
      ...
      str_detect(Term, "race_cat1") ~ "Race (ref: White): Am. Indian/Alaska Native",
      str_detect(Term, "race_cat3") ~ "Race (ref: White): Black or African American",
      str_detect(Term, "race_cat4") ~ "Ethnicity (ref: Non-Hispanic): Hispanic",
    ...
    Category = case_when(
      str_detect(Clean_Term, "Age|Sex|Race|Ethnicity|Rurality|ADI") ~ "1. Demographics",
      str_detect(Clean_Term, "AUD|MAUD|OUD|AUDIT-C") ~ "2. Substance Use History",
      str_detect(Clean_Term, "CAN|Frailty|ICU") ~ "3. Medical Complexity",
      str_detect(Clean_Term, "Consult|Speciality|Discharge") ~ "4. Hospitalization Factors",
      TRUE ~ "Exclude"
    )
  ) %>%
...
ordered_levels <- rev(c(
  ...
  "Sex (ref: Male): Female",
  "Race (ref: White): Am. Indian/Alaska Native", "Race (ref: White): Black or African American", "Ethnicity (ref: Non-Hispanic): Hispanic",
...
p_forest <- ggplot(clean_data, aes(x = OR, y = Clean_Term, color = Model)) +
  ...
  theme(
    ...
    strip.text.y = element_text(angle = 0, face = "bold", hjust = 0, size = 12),
